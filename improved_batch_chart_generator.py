...
# -*- coding: utf-8 -*-
"""
개선된 배치 차트 생성기 - 랭킹 종목들의 월봉 차트를 일괄 생성 (Unicode 인코딩 문제 해결)
"""
    ...
    try:
        ...
        for i, line in enumerate(lines, 1):
            ...
            try:
                ...
                # improved_month_stock_analysis.py 실행
                result = subprocess.run(
                    [sys.executable, 'improved_month_stock_analysis.py'],
                    input=stock_code + '\n',
                    text=True,
                    capture_output=True,
                    timeout=60,  # 60초 타임아웃
...
def main():
    ...
    # 1. 월간 거래대금 랭킹 차트 생성 (상위 10개만 먼저 테스트)
    print("\n🔥 1단계: 월간 거래대금 랭킹 상위 10개 종목 차트 생성")
    success1, error1, errors1 = generate_charts_from_file(trading_amount_file, "월간 거래대금 (상위 10개)")
    total_success += success1
    total_error += error1
    
    # 2. 월간 거래율 랭킹 차트 생성 (상위 10개만 먼저 테스트)
    print("\n🔥 2단계: 월간 거래율 랭킹 상위 10개 종목 차트 생성")
    success2, error2, errors2 = generate_charts_from_file(trading_rate_file, "월간 거래율 (상위 10개)")
